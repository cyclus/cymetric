...
# Quantity per GigaWattElectric Per Cumulative Power in TransactionQuantity [kg/GWe]
_tranactsdeps = ['TransactionQuantity','CumulativeTimeSeriesPower']
_tranactsschema = [
   ('SimId', ts.UUID),
   ('TransactionId', ts.INT),
   ('ResourceId', ts.INT),
...
@metric(name='TransactionQuantityPerCumulativePower', depends=_tranactsdeps, schema=_tranactsschema)
def transaction_quantity_per_cumulative_power(tranacts,cumpower):
   ...
   tranacts = pd.merge(tranacts,cumpower, on=['SimId','Time'],how='left')
