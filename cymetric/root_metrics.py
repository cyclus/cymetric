...
def _genrootclass(name, registry):
...
def root_metric(obj=None, name=None, schema=None, registery=None, *args, **kwargs):
    """Decorator that creates a root metric from a function or class."""
    if obj is not None:
        raise RuntimeError
    if name is None:
        raise RuntimeError
    return _genrootclass(name=name, registry=registery)
